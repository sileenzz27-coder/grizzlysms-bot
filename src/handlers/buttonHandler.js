...
const { getNumber, setStatus, getErrorMessage, get5SimNumber, finish5SimOrder, cancel5SimOrder, getHeroSmsNumber, finishHeroSmsOrder, cancelHeroSmsOrder, getSmspinverifyNumber, rejectSmspinverifyNumber, getSmspinverifyNumber4, rejectSmspinverifyNumber4, getHeroSmsPhysicNumber, cancelHeroSmsPhysicOrder, getSmsPoolNumber, cancelSmsPoolOrder } = require('../utils/grizzlyAPI');
...
async function handleButtonClick(interaction) {
  ...
  if (customId === 'ig_service_5_btn') {
    ...
    console.log(`[${new Date().toISOString()}] 🟣 Requesting HeroSMS Physic number for ${user.username}...`);
    const result = await getHeroSmsPhysicNumber(process.env.HEROSMS_API_KEY, '187');
    console.log(`[${new Date().toISOString()}] Result:`, result);
    ...
    startPolling(interaction.client, interaction, activationId, user.id, user.username, phoneNumber, 'herosms_physic');
    return;
  }
  ...
  if (customId === 'ig_service_6_btn') {
    ...
    console.log(`[${new Date().toISOString()}] 🟣 Requesting SMSPool number for ${user.username}...`);
    const result = await getSmsPoolNumber(process.env.SMSPOOL_API_KEY, 457, 1);
    console.log(`[${new Date().toISOString()}] Result:`, result);
    ...
    startPolling(interaction.client, interaction, orderId, user.id, user.username, phoneNumber, 'smspool');
    return;
  }
  ...
  if (customId.startsWith('num_cancel_')) {
    ...
    try {
      ...
      } else if (activation.provider === 'herosms_physic') {
        await cancelHeroSmsPhysicOrder(process.env.HEROSMS_API_KEY, activationId);
      ...
      } else if (activation.provider === 'smspool') {
